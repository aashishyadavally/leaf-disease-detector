...
def Classification_Results(filename):
    if request.method == 'GET':
        ...
        if len(class_ids) > 0:
            ...
            print(labels, format_confidences)
            return render_template('Classification-Results.html', confidences=format_confidences, labels=labels, 
                old_filename = filename,
                filename=new_filename)
        else:
            found = False
            # replace 'Objects' with whatever you are trying to detect
            return render_template('Classification-Results.html', labels='no objects', old_filename=filename, filename=filename) 
